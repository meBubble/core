...
const minRoot = 1 // minimal root peers to find when booting (2 should be minimum)
...
loopSeedList:
	for _, seed := range backend.Config.SeedList {
		...
		for _, addressA := range seed.Address {
			...
			backend.LogError("initSeedList", "peer added: '%s' \n", addressA)
		}
...
func (backend *Backend) bootstrap() {
	...
	for n := 0; n < 10*60/7; n++ {
		...
		if connected, total := countConnectedRootPeers(); connected == total || connected >= minRoot {
			return
		}
